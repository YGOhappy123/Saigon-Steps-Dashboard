...
import { AddDamageData, formSteps } from '@/features/damageReport/pages/AddDamagePage'
import { INVENTORY_DAMAGE_REASON_MAP } from '@/configs/constants'
import formatCurrency from '@/utils/formatCurrency'

type AddDamageFormFinalStepProps = {
    data: AddDamageData
    rootProducts: IRootProduct[]
    onConfirm: (values: AddDamageData) => Promise<void>
    onPrev: () => void
    isLoading: boolean
}

const AddDamageFormFinalStep = ({ data, rootProducts, onConfirm, onPrev, isLoading }: AddDamageFormFinalStepProps) => {
    return (
        <div>
            <Accordion type="multiple" className="w-full" defaultValue={['item-1']}>
                    ...
                    <AccordionContent>
                        <div className="flex flex-1 flex-col gap-4 p-4">
                            <div className="text-justify">
                                <span className="text-card-foreground font-medium">1.1. Phân loại: </span>
                                {INVENTORY_DAMAGE_REASON_MAP[data.reason]}
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                {formatCurrency(
                                    data.items.reduce((acc, curr) => acc + curr.quantity * curr.expectedCost, 0)
                                )}
                            </div>
                        </div>
                    </AccordionContent>
                ...
                <AccordionItem value="item-2">
                    ...
                    <AccordionContent className="py-4">
                        <ProductReviewTable rootProducts={rootProducts} reportItems={data.items} />
                    </AccordionContent>
                </AccordionItem>
            </Accordion>
...
type ProductReviewTableProps = {
    rootProducts: IRootProduct[]
    reportItems: IInventoryDamageReport['reportItems']
}

const ProductReviewTable = ({ rootProducts, reportItems }: ProductReviewTableProps) => {
    ...
    return <DataTable data={reportItems} columns={columns} />
}

export default AddDamageFormFinalStep
